package MYDan::UDPFileServer::Server;
...
=head1 SYNOPSIS
 ...
 my $server = MYDan::UDPFileServer::Server->new( ip => '0.0.0.0', port => 65111 );
 $server->run();
...
use base qw( MYDan::UDPFileServer );

our %RUN = ( %MYDan::UDPFileServer::RUN, Timeout => 6 );
